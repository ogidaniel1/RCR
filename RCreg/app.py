...
class Attendance(db.Model):
    ...
    marked_by_id = db.Column(db.Integer, db.ForeignKey('admin.id'), nullable=False)
    member = db.relationship('Member', backref='attendances')
    marked_by = db.relationship('Admin', backref='attendances')
...
@app.route('/deletion_logs')
def deletion_logs():
    # Check if current user is admin
    # if session['is_webmaster'] == False:
    #     abort(403)

    try:
        deletion_logs = DeleteLog.query.order_by(DeleteLog.deleted_at.desc()).all()
        if not deletion_logs:
            return render_template('deletion_logs.html', message="No deletion logs found.")
        return render_template('deletion_logs.html', deletion_logs=deletion_logs)
    except Exception as e:
        return render_template('deletion_logs.html', message=f"An error occurred: {e}")
...
def delete_member(member_id):
    ...
    try:
        ...
        Attendance.query.filter_by(member_id=member_id).delete()

        # Log the deletion
        log_action('delete_member', current_user.id, record_type='member', record_id=member.id)
